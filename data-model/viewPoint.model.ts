...
export class ViewPoint extends Typegoose {
  ...
  @staticMethod
  static loadComments(
    this: ModelType<ViewPoint> & typeof ViewPoint,
    id: string,
    skip: number,
    limit: number
  ) {
  ...
  @staticMethod
  static findViewPointsByCity(
    this: ModelType<ViewPoint> & typeof ViewPoint,
    cityId: string
  ) {
    return this.find({ city: cityId }).slice("comments", [
      0,
      ViewPoint.commentsFirstLoad
    ]);
  }
  @staticMethod
