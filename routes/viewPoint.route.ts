...
export class ViewPointRoute {
  ...
  private static async addComment(
    ...
    if (viewPoint == null)
      throw new Error("ViewPoint Id " + req.params.id + " not exist");
  ...
  private static async loadComments(
    ...
    var vp = await ViewPointModel.loadComments(
      req.params.id,
  ...
  private static async loadByCity(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
